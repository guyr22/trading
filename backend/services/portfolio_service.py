...
class PortfolioService:
    ...
    def _compute_positions_and_realized(self) -> tuple[list[PositionResponse], float]:
        """Single DB fetch + single FIFO pass.

        Returns (all_positions, total_realized_excl_index).
        """
        ...
        for ticker, trades in by_ticker.items():
            ...
            if ticker not in INDEX_TICKERS_SET:
                total_realized_excl_index += result.realized_pnl
            if qty == 0:
                continue
            held.append((ticker, qty, result.avg_cost))

        if not held:
            return [], total_realized_excl_index
        ...
        return positions, total_realized_excl_index
    ...
    def total_realized_pnl(self, exclude: set[str] | frozenset[str] | None = None) -> float:
        """Standalone realized P&L query — used externally when positions are not needed."""
        all_trades = self._trade_repo.get_all_ordered()
        if exclude:
    ...
    def build_summary(self) -> PortfolioSummary:
        ...
        return PortfolioSummary(
            total_market_value=round(sum(p.market_value for p in non_index), 2),
